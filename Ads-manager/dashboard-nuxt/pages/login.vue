...
async function submitLogin() {
  ...
  try {
    ...
    window.location.assign(`/dashboard${payload.redirectTo ?? dashboardRoutes.overview}`);
  } catch (error) {
...
onMounted(async () => {
  ...
  if (session) {
    window.location.assign(`/dashboard${dashboardRoutes.overview}`);
  }
});
</script>
      ...
      <div class="relative flex min-h-screen items-center justify-center p-6 sm:p-10">
        ...
        <div class="w-full max-w-md rounded-3xl border border-default bg-default p-6 shadow-sm sm:p-8">
          ...
          <form class="mt-8 space-y-5" data-testid="login-form" @submit.prevent="submitLogin">
            ...
            <UButton type="submit" color="primary" size="lg" block data-testid="login-submit" :loading="loginLoading" loading-icon="" @click="submitLogin">
